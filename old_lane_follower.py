...
class World(object):
    ...
    def restart(self):
        ...
        if self.player is not None:
            ...
            spawn_point.location.x = -123.505
            spawn_point.location.y = 136.603
            spawn_point.location.z += 1.0
            spawn_point.rotation.roll = 0.0
            spawn_point.rotation.pitch = 0.0
            spawn_point.rotation.yaw = 1.57
            self.destroy()
            self.player = self.world.try_spawn_actor(blueprint, spawn_point)
        while self.player is None:
            spawn_points = self.map.get_spawn_points()
            spawn_point = random.choice(spawn_points) if spawn_points else carla.Transform()
            spawn_point.location.x = -32.7
            spawn_point.location.y = -4.9
            spawn_point.location.z += 1.0
            spawn_point.rotation.roll = 0.0
            spawn_point.rotation.pitch = 0.0
...
def run_demo(args):
    ...
    try:
        ...
        # is enabled.
        world.world.tick()
        sim_start_stamp = float(world.world.wait_for_tick().elapsed_seconds) / 1000.0
        ...
        for i in range(num_iterations):
            ...
            if i == num_iterations - 1:
                sim_duration = float(world.world.wait_for_tick().elapsed_seconds) / 1000.0 -\
                               sim_start_stamp
        ...
        for frame in range(TOTAL_EPISODE_FRAMES):
            ...
            current_timestamp = float(world.world.wait_for_tick().elapsed_seconds) / 1000.0
