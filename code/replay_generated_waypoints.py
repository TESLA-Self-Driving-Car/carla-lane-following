...
def game_loop(args):
    ...
    try:
        ...
        while True:
            ...
            curr_yaw = math.degrees(world.player.get_transform().rotation.yaw)
            # waypoint = world.map.get_waypoint(curr_pose)
            # waypoints = world.map.generate_waypoints(100)
            # print(waypoints[0], len(file_waypoints))
            # waypoint = random.choice(waypoint.next(2))
            temp = file_waypoints[i%len(file_waypoints)]
            i += 1
            curr_location = carla.Location(x=temp[0], y=temp[1])
            curr_waypoint = world.map.get_waypoint(curr_location)
            # file_waypoint = carla.Transform(carla.Location(x=temp[0], y=temp[1], z=2.0), carla.Rotation(pitch=0, yaw=math.degrees(temp[2]), roll=0))
            world.player.set_transform(curr_waypoint.transform)
